...
for varNumber =1004
    ...
    switch varNumber
            ...
            subTitle = '{\bf Ising2D:} $32^2$ spins, $10^5$ MCSteps';
%             subTitle = '$(Z_2\!\times\! Z_2\!\times\! Z_2)^{*}_{124}$ Model, $32^2$ grid, $10^4$ sweeps, Bin $1$';
            delLegend=0;
            vN=1;varX{vN}=metropolis(:,1);leg{vN}='Metropolis';
            lT{vN}='o';lW{vN}=1;mS{vN}=4;lC{vN}=lineColor{1};
            switch varNumber
                case 1001
                    fileName = 'MetvsWolff_meanM2';
                    plotTitle = '\bf Mean Magnetization$^2$';
                    labelY1 = '$\left<M^2\right>$ ';moveY=12;
                    xprec='%1.1f';yprec='%1.1f';
                    yN=4;
                    varY{vN}=metropolis(:,yN);
                                vN=2;varX{vN}=wolff(:,1);leg{vN}='Wolff';
            lT{vN}='x';lW{vN}=1;mS{vN}=3.5;lC{vN}=lineColor{2};
                                yN=4;
                    varY{vN}=wolff(:,yN);
                    MINY=0;
                    
                case 1002
                    fileName = 'MetvsWolff_meanE';
                    plotTitle = '\bf Mean Energy';
                    labelY1 = '$\left<H\right>$ ';moveY=12;
                    xprec='%1.1f';yprec='%1.1f';
                    yN=6;
                    varY{vN}=metropolis(:,yN)+2;
                                vN=2;varX{vN}=wolff(:,1);leg{vN}='Wolff';
            lT{vN}='x';lW{vN}=1;mS{vN}=3.5;lC{vN}=lineColor{2};
                    ...
                    MINY=0;
                case 1003
                    fileName = 'MetvsWolff_c';
                    plotTitle = '\bf Mean Energy';
                    labelY1 = '$C/N$ ';moveY=12;
                    xprec='%1.1f';yprec='%1.1f';
                    yN=9;
                    varY{vN}=metropolis(:,yN);
                                vN=2;varX{vN}=wolff(:,1);leg{vN}='Wolff';
            lT{vN}='x';lW{vN}=1;mS{vN}=3.5;lC{vN}=lineColor{2};
                ...
                case 1004
                    fileName = 'MetvsWolff_chiabs';
                    plotTitle = '\bf Mean Energy';
                    labelY1 = '$\chi_{\rm abs}$ ';moveY=12;
...
%                     varY{vN}=log(abs(var(:,yN)));
            ...
            delLegend=0;
            vN=1;varX{vN}=metropolis(:,1);leg{vN}='$g(x)=g(\sigma_0,\sigma_x)$';
...
%          set(legH,'Xcolor',[1 1 1],'Ycolor',[1 1 1],'EdgeColor',[1 1 1],'color',[1 1 1]);
    ...
    set(yH, 'position', [yP(1)-moveY-8, yP(2)])
    set(xH, 'position', [xP(1), xP(2)-10])
    ...
    %Create Subtitle
    if useSubTitle==1
        subH = text(0.5 +0.00*axesWidth ,1+0.12/axesHeight,...
