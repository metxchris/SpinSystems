...
axesWidth  = 2.9*sizeScale;   %Width of Axes
        ...
        case {1,2,3,4,5,6,7,8,9} %2d,1d models
            ...
            switch varNumber
                case 1
                    ...
                    plotTitle = '\bf Mean Magnetization';
                    labelY1 = '$|\left<M\right>|$ ';
                    xprec='%1.1f';yprec='%1.1f';MINY=0;MINX=0.2;MAXX=2;
                    yN=4;moveY=8;
                    ...
                    if (plotMode ==1)
                        ...
                        vN=2;leg{vN}='$q=6$';var=Clock6;
                            varX{vN}=var(:,1); varY{vN}=smooth(var(:,yN));
                            lT{vN}='-';lW{vN}=1;mS{vN}=3;lC{vN}=lineColor{vN};
                        vN=3;var=Clock7;leg{vN}='$q=7$';
                            varX{vN}=var(:,1);varY{vN}=smooth(var(:,yN));
                            lT{vN}='-.';lW{vN}=1;mS{vN}=4;lC{vN}=lineColor{vN};
                        vN=4; var=Clock8;leg{vN}='$q=8$';
                            varX{vN}=var(:,1); varY{vN}=smooth(var(:,yN));
                            lT{vN}=':';lW{vN}=1;mS{vN}=3;lC{vN}=lineColor{vN};
                        vN=5;var=Clock27;leg{vN}='$q=27$';
                            varX{vN}=var(:,1);varY{vN}=smooth(var(:,yN));
                            lT{vN}='--';lW{vN}=1;mS{vN}=4;lC{vN}=lineColor{vN};
                        
                            yTickMarks=[0 0.4 0.8 1.2];
                    elseif (plotMode ==2)
                ...
                case 4
                    ...
                    xprec='%1.1f';yprec='%1.1f';MINX=0.2;MAXX=1.6;MINY=0;
                    yN=11;moveY=10;
...
%                     lT{vN}='-.';lW{vN}=2;mS{vN}=1;lC{vN}=lineColor{9};
  ...
  case {2001,2002,2003,2004,2005,2006,2007,2008,2009,2010} % Q8_0
            ...
            subTitle = '$J=1$, $32^2$ grid, $10^6 $ sweeps';MINX=0.199;MINY=0.199;
            delLegend=1;
...
%         plot([x x],[minY 1.05*maxY],':','linewidth',0.25,'markersize',4,'color',[0.75 0.75 0.75]);swapChildren=1;leg{vN+1} = '$T_c$';
    ...
    cmapline('ax',gca,'colormap',colormap(flipud(mycmap5)));colormap(flipud(colormap));c=colorbar('location','eastoutside');
...
%          set(legH,'Xcolor',[1 1 1],'Ycolor',[1 1 1],'EdgeColor',[1 1 1],'color',[1 1 1]);
    ...
    %Create Subtitle
    if useSubTitle==1
        subH = text(0.5 -0.04*axesWidth ,1+0.07/axesHeight,...
